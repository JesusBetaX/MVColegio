...
class DB 
{
  ...
  public function insert($table, $obj) {
    ...
    $sql = 'INSERT INTO '.$table.' ('.implode(', ', $keys).') VALUES ('.implode(', ', $args).')';
...
class Cursor implements \Iterator
{
  ...
  public function fetchOne($row_factory = NULL) {
    if (!is_null($row_factory)) $this->row_factory = $row_factory;
  ...
  public function fetchMany($size) {
    $array = array();
    for ($i = 0; $i < $size; $i++) {
      $array[$i] = $this->statement->fetchObject($this->row_factory);
    }
    return $array;
  }

  public function fetchAll($row_factory = NULL) {
    if (!is_null($row_factory)) $this->row_factory = $row_factory;
